...
window.addEventListener("scroll", () => {
  ...
  const leftTranslate = window.scrollY / 3.5;
  const rightTranslate = -window.scrollY / 6.5;
  headTranslateLeft.forEach(head => {
    head.style.transform = "translateX(" + leftTranslate + "%)"
  });
  headTranslateRight.forEach(head => {
    head.style.transform = "translateX(" + rightTranslate + "%)"
  });
})
